...
class VideoRotator:
    ...
    def build_mapping(self, video_dir: str, target_start: str, target_end: str,
        ...
        if num_videos <= num_dirs:
            ...
            # 模式2: 视频数 > 目录数,滑动窗口分配
            window_size = 1  # 每个视频对应1个目录
            mode = "sliding_window"
            max_rotations = num_videos  # 旋转次数 = 视频数,让每个视频都能覆盖所有目录
        ...
        if mode == "video_to_dirs":
            ...
            logger.info(f"  映射模式: 滑动窗口 (每个目录1个视频)")
            logger.info(f"  最大旋转次数: {max_rotations} (每个视频覆盖所有目录)")
            logger.info(f"  窗口大小: {num_dirs} 个目录")
    ...
    def _print_current_mapping(self, ring_name: str):
        ...
        if mode == "video_to_dirs":
            ...
            for i in range(display_count):
                ...
                # 计算该目录对应的视频: (目录索引 + 偏移) % 视频总数
                video_idx = (i + offset) % len(videos)
                video_name = os.path.basename(videos[video_idx])
    ...
    def rotate(self, ring_name: str) -> bool:
        ...
        if mode == "video_to_dirs":
            ...
            # sliding_window 模式: 偏移按视频总数循环
            new_offset = (current_offset + 1) % len(videos)
        ...
        if mode == "video_to_dirs":
            ...
            for i, target_dir in enumerate(target_dirs):
                video_idx = (i + new_offset) % len(videos)
